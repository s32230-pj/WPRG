<?php
$ilosc_osob = isset($_POST['ilosc_osob']) ? $_POST['ilosc_osob'] : 1;
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!$blad) {
        ...
        $rezerwacja = [
            ...
            'Adres' => $adres,
            'Email' => $email,
            'Karta kredytowa' => $karta_kr,
            'Ilość osób' => $ilosc_osob,
            'Data pobytu' => $data_pobytu,
            'Godzina przyjazdu' => $godzina_przyjazdu,
            'Łóżko dla dziecka' => $lozko_dla_dziecka,
        ...
        if (file_exists($csv_file)) {
            ...
            fputcsv($file, array_keys($rezerwacja)); // Nagłówki
        }

        fputcsv($file, array_values($rezerwacja)); // Zapisanie danych
        fclose($file);
    }
}
?>
...
<form method="post">
    ...
    <select name="ilosc_osob" id="ilosc_osob" required onchange="this.form.submit()">
        <option value="1" <?php echo ($ilosc_osob == 1) ? 'selected' : ''; ?>>1</option>
        <option value="2" <?php echo ($ilosc_osob == 2) ? 'selected' : ''; ?>>2</option>
        <option value="3" <?php echo ($ilosc_osob == 3) ? 'selected' : ''; ?>>3</option>
        <option value="4" <?php echo ($ilosc_osob == 4) ? 'selected' : ''; ?>>4</option>
    </select><br><br>
